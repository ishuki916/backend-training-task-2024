...
// 請文字解釋為什麼
console.log(k * l > 21);
//true, 原因：l的值"8"，在乘法運算中轉型成number型別，3和8相乘後變成24，因此大於21
...
let gameSource = ["剪刀", "石頭", "布"];
if (!gameSource.includes(playerA) || !gameSource.includes(playerB)) {
  console.log("無法判定");
...
const activities = {
  ...
  yoga: { name: "瑜伽", calorieBurn: 5 },
  cycling: { name: "騎腳踏車", calorieBurn: 8 },
...
// 練習：計算小明今日消耗的卡路里
calorieBurn =
  activities.cycling.calorieBurn * 10 +
  activities.treadmill.calorieBurn * 30 +
  activities.yoga.calorieBurn * 40 +
  activities.cycling.calorieBurn * 10;
...
// 練習：週二、週三、週四、週五、週六
exerciseRecords.forEach((valule, index) => {
  if (
    exerciseRecords[index].duration >= 30 &&
    exerciseRecords[index].heartRate >= 130
  ) {
    console.log(index);
    totalDuration += exerciseRecords[index].duration;
    validDays += 1;
  }
});
console.log(`totalDuration:${totalDuration} validDays: ${validDays}`);
// 練習：判斷是否符合 533 原則
